...
CREATE TABLE ai_consultations (
  ...
  category VARCHAR(100), -- 키워드 카테고리 (부당인사조치, 퇴직금체불 등)
  ...
  follow_up_required BOOLEAN DEFAULT false, -- 후속 상담 필요 여부
...
CREATE TABLE ai_messages (
  ...
  content TEXT NOT NULL,
  metadata JSONB, -- 추가 메타데이터 (토큰 수, 처리 시간 등)
...
CREATE TABLE consultation_categories (
  id SERIAL PRIMARY KEY,
  name VARCHAR(100) NOT NULL UNIQUE, -- 카테고리명 (부당인사조치, 퇴직금체불 등)
  display_name VARCHAR(100) NOT NULL, -- 화면 표시용 이름
  description TEXT,
  keywords TEXT[], -- 관련 키워드 배열
  form_template JSONB, -- 폼 템플릿 정보 (각 카테고리별 질문 양식)
  color VARCHAR(20) DEFAULT '#3B82F6', -- 태그 색상
  icon VARCHAR(50), -- 아이콘 이름 (lucide 아이콘)
...
### ✅ 완료된 기능
- 메인 페이지 히어로 섹션 구글 검색창 스타일로 변경
- AI 상담 시스템 (Google Gemini API 연동)
- 실시간 채팅 인터페이스
- 상담 내역 데이터베이스 저장
- 관리자 상담 관리 페이지
- 카테고리별 전문 상담 지원
...
### 🎯 카테고리별 상담 분야
1. **부당인사조치** (빨간색 🚨) - 해고, 징계 관련
2. **퇴직금체불** (주황색 💰) - 임금, 퇴직금 미지급
3. **직장내괴롭힘** (보라색 🛡️) - 성희롱, 갑질 관련
4. **산재상담** (초록색 ❤️) - 업무상 재해, 산재보험
5. **근로계약서** (파란색 📄) - 계약서 검토, 근로조건
